...
cmake -B build -DBUILD_SHARED_LIBS=OFF -DLLAMA_METAL=ON -DLLAMA_CURL=OFF -DLLAMA_OPENSSL=OFF -DCMAKE_OSX_ARCHITECTURES="arm64" -DGGML_NATIVE=OFF -DLLAMA_NATIVE=OFF && cmake --build build -j --config Release
